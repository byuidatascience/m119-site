...
- The submitted `.html` file is **self-contained**. Set `embed-resources: true` in your YAML header so plots, images, and other assets are inlined into the single `.html` file. Without this, Quarto saves your plots into a sibling `_files/` folder and the `.html` references them externally — uploading just the `.html` to Canvas means your grader sees broken images. See [Quarto Hints](QuartoHints.qmd) for a working YAML template.
