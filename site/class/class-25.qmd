...
The base has area $x^2$. Letting $h$ be the height of the box, the volume of the box is $V(x,h) = x^2h$.  Each the sides has area $4xh$, which means the total area of 6 must satistify $6 = x^2+4xh$. From here we can get $h$ by itself as $h =\frac{6-x^2}{4x}$.  This means the volume is $$
\begin{aligned}
V(x) &= x^2 h \\
     &= x^2 \, \frac{6 - x^2}{4x} \\
     &= \frac{1}{4} x (6 - x^2).
...
2.  Use R to plot the function above, and then guess the maximum possible volume along with the dimensions that correspond to this volume.
...
3. Compute the derivative of the function above, and use it to find the maximum possible volume.
...
This worksheet walks you from $f$ to $f'$ to $f''$ on the same axes, one step at a time. For each example below:

1. **Sketch $f(x)$** on paper using the formula given.
2. Mark the **zeros of $f$** on your sketch with **filled (black) dots** on the $x$-axis.
...
After each step, run the matching code block to **check your work**. Colors stay consistent across examples: $f$ in black, $f'$ in red, $f''$ in blue.

Reusable plotting helper — run this once at the top of your R session:

```r
# Plot up to three of {f, f', f''} on shared axes, with optional dots.
...
Start simple. Sketch the parabola first, then check.

<details>
<summary>Check Step 1 — zeros of $f$ (filled black dots)</summary>

$f(x) = x^2 = 0$ only at $x = 0$ (the vertex of the parabola touches the $x$-axis).
...
<details>
<summary>Check Step 2 — inflection points + overlay $f'$</summary>

$f'(x) = 2x$ — a straight line through the origin.
$f''(x) = 2$ — a constant. **$f''$ is never zero**, so $f$ has **no inflection points** (no open dots).
...
Notice: $f'$ (red) is **negative** where $f$ slopes down, **positive** where $f$ slopes up, and **zero** at the vertex (the minimum of $f$).

</details>

<details>
<summary>Check Step 3 — overlay $f''$</summary>

$f''(x) = 2$. A flat horizontal line — $f$ is **concave up everywhere**, which is exactly why there are no inflection points.
...
Takeaway: $f'' > 0$ everywhere $\Longleftrightarrow$ $f$ is concave up everywhere.
...
Same triple zero at the origin, but now watch how $f$, $f'$, $f''$ interact differently.

<details>
<summary>Check Step 1 — zeros of $f$</summary>

$f(x) = x^3 = 0$ only at $x = 0$ (a triple zero — the curve passes through the axis and momentarily flattens against it).
...
$f'(x) = 3x^2$ — always **non-negative**. So $f$ is **never decreasing**: it slopes upward everywhere except for the single flat instant at $x = 0$.
$f''(x) = 6x$ has a sign change at $x = 0$, so $x = 0$ **is** an inflection point (open dot).
...
**Important teaching moment:** $f'(0) = 0$ but $x = 0$ is **not** a max or min — it's a *horizontal inflection*. A zero of $f'$ only gives a max or min when $f'$ also **changes sign** there.
...
$f''(x) = 6x$. Negative for $x < 0$ (concave down), positive for $x > 0$ (concave up). It crosses zero at the open dot, confirming $x = 0$ is the inflection point.
...
$f(x) = x^4 = 0$ only at $x = 0$ (a quadruple zero — the curve sits tangent to the axis).
...
$f'(x) = 4x^3$ — negative for $x < 0$, positive for $x > 0$, zero at $x = 0$ (the **minimum** of $f$).
$f''(x) = 12x^2$ — equals zero at $x = 0$, **but $f'' \ge 0$ everywhere**. No sign change → **not** an inflection point.
...
**Important teaching moment:** $f''(c) = 0$ does *not* automatically mean $c$ is an inflection point. You also need $f''$ to **change sign** there. Compare with Example B where it did.
...
$f''(x) = 12x^2$. It's a parabola that *touches* zero at $x = 0$ but never goes negative — confirming why $x = 0$ isn't an inflection point.
...
$e^x > 0$ for every real $x$, so $f$ has **no zeros** — no filled dots.
...
$f'(x) = e^x$ — **identical to $f$**. So the red curve lands exactly on the black curve.
$f''(x) = e^x$ too — never zero, no inflection points.
...
plot_derivs(x, f, fp = fp,
            ...
            inflections = NULL,
            ylim = c(0, 20),
            main = "f and f' (they're identical)")
```
...
$f''(x) = e^x$ again — the third curve also coincides with the first two. All three plots stack perfectly on top of each other.
...
Takeaway: $e^x$ is the unique function (up to a constant multiple) whose derivative equals itself — always positive, always increasing, always concave up.
...
Different territory: the domain is restricted to $x > 0$, and $f$ is monotonic and concave down.

<details>
<summary>Check Step 1 — zeros of $f$</summary>

$\ln(x) = 0$ exactly at $x = 1$. (Don't forget: $\ln$ is undefined for $x \le 0$.)
...
$f'(x) = 1/x$ — always **positive** on the domain, so $f$ is always increasing. No zeros of $f'$.
$f''(x) = -1/x^2$ — always **negative**. Never zero, so **no inflection points**.
...
Notice: as $x$ grows, $f'$ shrinks toward zero — that's why $\ln(x)$ keeps increasing but more and more slowly.
...
$f''(x) = -1/x^2$. Negative everywhere on the domain → $f$ is **concave down everywhere**, which matches the visible curvature of $\ln(x)$.
...
**Going further.** Once the pattern feels natural:

- Try a function whose zeros of $f$, $f'$, and $f''$ don't line up at all — e.g. $f(x) = x^3 - 3x$. Three real zeros of $f$, an inflection point at $x = 0$, and local extrema at $x = \pm 1$.
