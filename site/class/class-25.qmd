...
We graph $V(x) = \frac{1}{4}x(6 - x^2)$. The domain is $0 < x < \sqrt{6}$ (the side length must be positive, and we also need $h = (6-x^2)/(4x) > 0$).

```r
V <- function(x){(1/4) * x * (6 - x^2)}

#Plot V across its domain.
x <- seq(0.01, sqrt(6), 0.01)
plot(x, V(x), type = "l")
...
From the plot, the maximum volume is somewhere around $1.4$ cubic feet, at a side length a bit less than $x = 1.5$ feet. We'll pin down the exact value with derivatives in Question 3.
