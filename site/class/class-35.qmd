...
For this Unit, the reading as well as many practice exercises appear in the document [Rectangles, Targets, and Sums](../projects/probability.html){target="_blank"} 
...
10. The value $k$ is always $k =\frac{1}{A}$, which gives $k = \frac{1}{11}$ and so $$\ds f(x) = \begin{cases}\frac{4}{11}&0\leq x\leq 2\\\frac{1}{11}&2 < x\leq 5 \\ 0 &\text{otherwise} \end{cases}$$ 

11. There are two ways to finish this.  One is to use the original graph and from it we obtain $P(X\leq 3) = \frac{8+1}{11} = \frac{9}{11}$ where the numerator is the area to the left of 3.  The second option is to use the graph of $f$, and then compute areas using it.  Using the formula for $f$, the area to the left of 3 is $(2)\frac{4}{11} + (1)\frac{1}{11}=\frac{9}{11}$.

12. Calculate the expected value of $X$. We have lots of options, either using $g$ with the formula $\frac{\sum x_i A_i}{\sum A_i}$, or use $f$ with the formula $\sum x_i A_i$. The normalized function $f$ removes the need to divide by the total area.   
...
14. The derivative of $F$, where it is nonzero and defined, is $$\ds F'(x) = \begin{cases}\frac{4}{11}&0< x< 2\\\frac{1}{11}&2< x< 5 \end{cases}$$ 
